...
// Low Loglevel means that less important logs will be displayed
	...
	#define LOG_DEBUG	ACTK::Core::EventLogger::GetInstance().LogDebug
#else
	#define LOG_DEBUG
#endif

#if LOG_LEVEL <= LOG_LEVEL_WARNING
	#define LOG_WARNING	ACTK::Core::EventLogger::GetInstance().LogWarning
#else
	#define LOG_WARNING
#endif

#if LOG_LEVEL <= LOG_LEVEL_ERROR
	#define LOG_ERROR	ACTK::Core::EventLogger::GetInstance().LogError
#else
	#define LOG_ERROR
#endif

#define LOG_FATAL(var_1)			ACTK::Core::EventLogger::GetInstance().LogAssert(false, __FILE__, __LINE__, var_1)
#define LOG_ASSERT(var_1, var_2)	ACTK::Core::EventLogger::GetInstance().LogAssert(var_1, __FILE__, __LINE__, var_2)
